...
Original file is located at
    https://colab.research.google.com/drive/1033C_05iaaEyO-WOsuvYEnIiZmkBQQuT
...
results = {}
predictions = {}
histories = {}  # Store the training history for each horizon
...
for h in horizons:
    ...
    gru_model = tf.keras.Sequential([
        tf.keras.layers.GRU(units=100, activation='relu', input_shape=(X_train.shape[1], X_train.shape[2])),
    ...
    history = gru_model.fit(X_train, y_train, epochs=100, batch_size=64, validation_split=0.15, shuffle=False)
    histories[h] = history  # Save the training history
    ...
    # Evaluation
    def GRU_accuracy_metrics(y_true, y_pred):
    ...
    metrics = GRU_accuracy_metrics(y_test_inv, y_pred_inv)
    results[h] = metrics
...
for i, h in enumerate(horizons):
    ...
    plt.plot(histories[h].history['loss'], label='Training Loss')
    plt.plot(histories[h].history['val_loss'], label='Validation Loss')
...
plt.suptitle('GRU Model Loss Across Horizons', fontsize=16)
...
fig.suptitle('GRU GHI Forecasting Comparison', fontsize=17)
...
print("\nGRU Comparison of Metrics Across Horizons")
for h in horizons:
    print(f"\n{h}-Hour Ahead:")
...
plt.title('GRU RMSE Comparison Across Forecasting Horizons')
...
# Add a master title for the entire figure
fig.suptitle('GRU Regression Plot', fontsize=17)
...
# Save to CSV
results_df.to_csv('GRU07a_ghi_predictions.csv', index=False)

print("CSV file 'GRU07a_ghi_predictions.csv' has been created successfully.")
